  ...
  authenticate(req) {
    ...
    if (req.headers && req.headers.authorization) {
      ...
      if (parts.length === 2) {
        ...
        return oauthdoneScope.fail(400, 'Invalid bearer token provided');
      }
    }

    if (req.body && req.body.access_token) {
      if (token) {
        return oauthdoneScope.fail(400, 'Multiple tokens provided');
      }
    ...
    if (!token) {
      return oauthdoneScope.fail(400, 'No token provided');
    }
    ...
    function callback(err, user, info = {}) {
      ...
      if (!user) {
        ...
        return oauthdoneScope.fail(400, 'invalid_token');
      }

      oauthdoneScope.logIn(user, info);
    }
